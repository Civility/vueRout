...
export default {
  ...
  actions: {
    async createOrder ({commit}, {name, phone, productId, ownerId}) {
      ...
      commit('clearError')
      // set Loading не вызывается, т.к. есть свой лоадинг
      try {
  ...
  getters: {
    ...
    orders (state, getters) {
      return getters.undoneOrders.concat(getters.doneOrders) // undoneOrders не выполненые заказы, doneOrders - выполненые заказы
    }
  }
}
